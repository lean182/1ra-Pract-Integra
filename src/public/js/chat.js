...
Swal.fire({
   title: 'Identifícate',
   input: 'email',
   text: 'Ingresá tu correo electrónico para identificarte',
   inputValidator: value => {
      if (!value) return 'Necesitás ingresar un correo electrónico para continuar';

      const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;

      if (!emailRegex.test(value)) return 'Ingresá un correo electrónico válido';
